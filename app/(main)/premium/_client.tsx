...
function PlanCard({ plan, isCurrentPlan, onSelect, loading }: {
  plan: typeof PLANS[number]; isCurrentPlan: boolean;
  onSelect: (id: PlanId) => void; loading: boolean;
}) {
  return (
    <div className={`relative rounded-3xl border overflow-hidden transition-all duration-200 ${plan.popular ? "scale-[1.02]" : "border-white/8 opacity-90"}`}
      style={{
      ...
      {plan.popular && (
        <div className="absolute top-0 right-5 text-white text-[11px] font-bold px-3 py-1 rounded-b-xl shadow-lg"
          style={{ background: "linear-gradient(135deg,#8B5CF6,#7C3AED)" }}>⭐ POPULAIRE</div>
      )}
      <div className="p-5">
        <div className="flex items-start gap-3 mb-4">
          ...
          <div className="flex-1">
            ...
            <span className={`inline-block text-[10px] font-semibold px-2 py-0.5 rounded-full border mt-1 ${plan.badgeColor}`}>{plan.badge}</span>
          </div>
        </div>
        <div className="mb-5">
          ...
          <p className="text-[12px] text-white/30 mt-1">puis <span className="text-white/50 font-semibold">{plan.priceNormal}€/mois</span></p>
        </div>
        <div className="flex flex-col gap-2.5 mb-5">
          {plan.features.map((f, i) => (
            <div key={i} className="flex items-start gap-2.5">
              <div className={`w-4 h-4 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5 ${f.included ? "bg-[#8B5CF6]/20 border border-[#8B5CF6]/40" : "bg-white/5 border border-white/10"}`}>
                {f.included ? <Check className="w-2.5 h-2.5 text-[#A78BFA]" /> : <span className="w-1.5 h-0.5 rounded-full bg-white/20 block" />}
              </div>
              <span className={`text-[13px] leading-snug ${f.included ? "text-white/80" : "text-white/25 line-through"}`}>{f.text}</span>
            </div>
          ))}
        </div>
        {isCurrentPlan ? (
          <div className="w-full py-3.5 rounded-2xl border border-white/12 bg-white/5 text-center text-[14px] font-semibold text-white/40">✓ Plan actuel</div>
        ) : (
          <button onClick={() => onSelect(plan.id as PlanId)} disabled={loading}
            ...
            style={{ background: `linear-gradient(135deg,${plan.accentColor},${plan.accentColor}cc)`, boxShadow: `0 8px 24px ${plan.accentColor}30` }}>
            {loading ? <span className="w-5 h-5 animate-spin rounded-full border-2 border-white/30 border-t-white" /> : <>Choisir {plan.name} →</>}
          </button>
        )}
      </div>
    </div>
  ...
  return (
    <div className="min-h-[100dvh] bg-[#080810] flex flex-col pb-28">
      ...
      <div className="px-5 pt-3 pb-6 text-center">
        ...
        <h1 className="text-[28px] font-black text-white leading-tight">Vendez plus,<br />gagnez plus 🚀</h1>
        <p className="text-[14px] text-white/40 mt-2.5 leading-relaxed">Débloquez les outils IA pour vendre<br />plus vite et gagner plus.</p>
      </div>
      {usage && (
        <div className="mx-4 mb-5 rounded-2xl border border-white/8 bg-white/4 px-4 py-3 flex items-center justify-between">
          ...
          {usage.limit === null && <span className="text-[12px] font-bold text-amber-400 bg-amber-400/15 border border-amber-400/25 px-2.5 py-1 rounded-full">♾️ Illimité</span>}
        </div>
      )}
      {success && (
      ...
      <div className="px-4 flex flex-col gap-4">
        {PLANS.map(plan => (
          <PlanCard key={plan.id} plan={plan} isCurrentPlan={usage?.plan === plan.id}
            onSelect={handleSelect} loading={loadingPlan === plan.id} />
        ))}
      </div>
      <div className="px-5 pt-6 text-center">
        <p className="text-[11px] text-white/20 leading-relaxed">Résiliation possible à tout moment · Sans engagement<br />Paiement sécurisé · Prix TTC</p>
      </div>
    </div>
  );
}
