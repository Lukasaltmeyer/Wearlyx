  ...
  return (
    <div
      ...
      className={`flex flex-col items-center justify-center gap-4 rounded-3xl border-2 border-dashed p-10 cursor-pointer transition-all duration-200 ${
        dragging ? "border-[#8B5CF6] bg-[#8B5CF6]/10 scale-[0.99]" : "border-white/12 bg-white/4 hover:border-white/22 hover:bg-white/6"
      }`}
    >
      <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-[#8B5CF6]/20 to-[#A78BFA]/20 border border-[#8B5CF6]/25 flex items-center justify-center">
  ...
  return (
    <div className="flex flex-col items-center gap-5 py-10">
      <div className="relative w-20 h-20">
        <div className="absolute inset-0 rounded-full bg-[#8B5CF6]/15 animate-pulse" />
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="flex gap-2">
        ...
        <button onClick={download}
          className="flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl bg-[#8B5CF6] text-[14px] font-semibold text-white shadow-lg shadow-[#8B5CF6]/25 transition-all active:scale-[0.98]">
      ...
      <div className="flex items-center gap-3 px-4 pt-5 pb-4">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-2">
            <div className="w-6 h-6 rounded-lg bg-gradient-to-br from-[#8B5CF6] to-[#A78BFA] flex items-center justify-center">
        ...
        {/* Plan badge */}
        <span className="text-[10px] font-bold px-2 py-1 rounded-full bg-[#8B5CF6]/15 text-[#A78BFA] border border-[#8B5CF6]/25 flex-shrink-0">
          🆓 5/mois
        </span>
      </div>
      ...
      <div className="flex-1 px-4 flex flex-col gap-4">
        ...
        {photos.length > 0 && !processing && !hasResults && (
          <>
              ...
              {photos.map((f, i) => (
                <div key={i} onClick={() => setSelectedIdx(i)}
                  className={`relative flex-shrink-0 w-20 h-20 rounded-2xl overflow-hidden border-2 transition-all cursor-pointer ${
                    selectedIdx === i ? "border-[#8B5CF6]" : "border-white/10"
                  }`}>
            ...
            <button
              onClick={() => enhance()}
              className="w-full py-4 rounded-2xl bg-gradient-to-r from-[#8B5CF6] to-[#A78BFA] text-[15px] font-bold text-white shadow-lg shadow-[#8B5CF6]/25 transition-all active:scale-[0.98] flex items-center justify-center gap-2"
            >
                ...
                {results.map((r, i) => (
                  <button key={i} onClick={() => setSelectedIdx(i)}
                    className={`flex-shrink-0 w-16 h-16 rounded-xl overflow-hidden border-2 transition-all ${
                      selectedIdx === i ? "border-[#8B5CF6]" : "border-white/10 opacity-60"
                    }`}>
