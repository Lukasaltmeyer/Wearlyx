  ...
  const unsubscribeBlock = unsubscribeToken
    ? `<tr><td style="padding:24px 32px;text-align:center;border-top:1px solid #1a1a2e;">
        <p style="color:#555;font-size:11px;margin:0;">
          ...
          <a href="${APP_URL}/api/emails/unsubscribe?token=${unsubscribeToken}" style="color:#6C3AED;">Se désinscrire</a>
          &nbsp;·&nbsp;
          <a href="${APP_URL}/profile/settings/notifications" style="color:#6C3AED;">Gérer mes préférences</a>
        </p>
       </td></tr>`
    : `<tr><td style="padding:16px 32px;text-align:center;border-top:1px solid #1a1a2e;">
        <p style="color:#555;font-size:11px;margin:0;">Email transactionnel — <a href="${APP_URL}/profile/settings/notifications" style="color:#6C3AED;">Gérer mes notifications</a></p>
       </td></tr>`;
  ...
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#0a0a15;padding:32px 16px;">
    <tr><td align="center">
      <table width="100%" style="max-width:560px;background:#0f0f1a;border-radius:20px;overflow:hidden;border:1px solid #1a1a2e;">
        <!-- Header -->
        <tr><td style="background:linear-gradient(135deg,#6C3AED,#C026D3);padding:28px 32px;">
...
function buildContent(type: EmailType, data: Record<string, string | number> = {}): { subject: string; html: string } {
  ...
  const btn = (url: string, label: string) =>
    `<div style="margin:24px 0 0;"><a href="${url}" style="display:inline-block;background:linear-gradient(135deg,#6C3AED,#C026D3);color:white;font-weight:800;font-size:14px;text-decoration:none;padding:14px 28px;border-radius:12px;">${label}</a></div>`;
  ...
  switch (type) {
    ...
    case "marketing_promo":
      return {
        ...
        html: baseTemplate(
          `<div style="background:linear-gradient(135deg,#6C3AED22,#C026D322);border:1px solid #6C3AED44;border-radius:16px;padding:24px;margin:0 0 20px;text-align:center;">
