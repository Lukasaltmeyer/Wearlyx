  ...
  return (
    <div className="flex flex-col gap-4 animate-fadeIn">
      ...
      <div className="flex items-center gap-2">
        <div className="w-8 h-8 rounded-xl bg-[#22C55E]/20 border border-[#22C55E]/30 flex items-center justify-center">
        ...
        <div className="flex gap-2 overflow-x-auto no-scrollbar pb-1">
          {photos.map((f, i) => (
            <img
              ...
              className={`flex-shrink-0 rounded-2xl object-cover border ${i === 0 ? "w-24 h-24 border-[#22C55E]/40" : "w-16 h-16 border-white/10 opacity-70"}`}
              alt=""
            />
          ))}
        </div>
        ...
        {editing === "title" ? (
          <div className="flex flex-col gap-2">
            <input value={draft.title} onChange={(e) => setDraft({ ...draft, title: e.target.value })}
              className="w-full bg-white/8 border border-white/15 rounded-xl px-3 py-2 text-sm text-white outline-none focus:border-[#22C55E]/50" />
            <button onClick={save} className="self-end text-xs text-[#4ADE80] font-semibold">Sauvegarder</button>
          </div>
        ) : (
        ...
        {editing === "description" ? (
          <div className="flex flex-col gap-2">
            <textarea value={draft.description} onChange={(e) => setDraft({ ...draft, description: e.target.value })}
              rows={4}
              className="w-full bg-white/8 border border-white/15 rounded-xl px-3 py-2 text-sm text-white/80 outline-none focus:border-[#22C55E]/50 resize-none" />
            <button onClick={save} className="self-end text-xs text-[#4ADE80] font-semibold">Sauvegarder</button>
          </div>
        ) : (
      ...
      <div className="flex gap-2">
        <div className="flex-1 rounded-2xl border border-white/8 bg-white/4 p-3.5">
          ...
          <p className="text-[10px] text-[#4ADE80] mt-0.5">Prix conseillé par IA</p>
        </div>
        <div className="flex-1 rounded-2xl border border-white/8 bg-white/4 p-3.5">
          <div className="flex items-center gap-1.5 mb-1">
      ...
      <div className="flex gap-2 pt-1">
        ...
        <button
          onClick={onPublish}
          disabled={publishing}
          className="flex-1 flex items-center justify-center gap-2 py-3 rounded-2xl bg-[#22C55E] text-[15px] font-semibold text-white shadow-lg shadow-[#22C55E]/25 transition-all active:scale-[0.98] disabled:opacity-60"
        >
          {publishing ? (
