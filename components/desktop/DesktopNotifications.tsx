      ...
      <div style={{
        ...
        <div style={{ padding: "20px 20px 18px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {LIVE_SALES.map(s => (
              <div key={s.user} style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <div style={{
                  width: 32, height: 32, borderRadius: "50%", flexShrink: 0,
                  display: "flex", alignItems: "center", justifyContent: "center",
                  fontSize: 12, fontWeight: 800, color: "white",
                  background: `linear-gradient(135deg,${s.color}60,${s.color}30)`,
                }}>
                  {s.user[0].toUpperCase()}
                </div>
                <div style={{ flex: 1, minWidth: 0 }}>
                  <p style={{ fontSize: 12, fontWeight: 500, color: "rgba(255,255,255,0.55)", margin: "0 0 1px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{s.item}</p>
                  <p style={{ fontSize: 10.5, color: "rgba(255,255,255,0.2)", margin: 0 }}>@{s.user}</p>
                </div>
                <span style={{ fontSize: 12.5, fontWeight: 700, color: s.color, flexShrink: 0 }}>{s.price}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ padding: "18px 20px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "rgba(255,255,255,0.22)", margin: "0 0 12px" }}>
            Live · Wearlyx
          </p>
          {[
            ...
            <div key={label} style={{
              ...
              <span style={{ fontSize: 12.5, fontWeight: 600, color: "rgba(255,255,255,0.65)" }}>{value}</span>
            </div>
          ))}
        </div>
        ...
        <div style={{ padding: "18px 20px", borderBottom: "1px solid rgba(255,255,255,0.06)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 7, marginBottom: 10 }}>
            ...
            <p style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", color: "rgba(255,255,255,0.22)", margin: 0 }}>
              Tendances
            </p>
          </div>
          {TRENDING_NOW.map((t, i) => (
            <Link key={t} href={`/search?q=${encodeURIComponent(t)}`} style={{ textDecoration: "none" }}>
              <div style={{
                ...
                <span style={{ fontSize: 10, fontWeight: 700, width: 14, flexShrink: 0, textAlign: "center", color: i < 3 ? "rgba(139,92,246,0.6)" : "rgba(255,255,255,0.14)" }}>
                  {i + 1}
                </span>
                <span style={{ fontSize: 12.5, fontWeight: 500, flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", color: "rgba(255,255,255,0.45)" }}>
                  {t}
                </span>
                {i < 3 && (
                  <span style={{ fontSize: 10, fontWeight: 600, color: "rgba(167,139,250,0.55)", flexShrink: 0 }}>
                    ↑{["12%","8%","5%"][i]}
                  </span>
                )}
              </div>
        ...
        <div style={{ padding: "16px 20px", marginTop: "auto" }}>
          <Link href="/promotion-tools" style={{ textDecoration: "none" }}>
            <div style={{
              display: "flex", alignItems: "center", gap: 12, padding: "14px 16px", borderRadius: 12,
              background: "rgba(124,58,237,0.08)", border: "1px solid rgba(139,92,246,0.15)",
              transition: "all 0.12s", cursor: "pointer",
            }}
              onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = "rgba(124,58,237,0.14)"; (e.currentTarget as HTMLElement).style.borderColor = "rgba(139,92,246,0.28)"; }}
              onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = "rgba(124,58,237,0.08)"; (e.currentTarget as HTMLElement).style.borderColor = "rgba(139,92,246,0.15)"; }}>
              <div style={{
                width: 36, height: 36, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "center",
                background: "rgba(139,92,246,0.2)", border: "1px solid rgba(139,92,246,0.25)", flexShrink: 0,
              }}>
                <Zap style={{ width: 16, height: 16, color: "#A78BFA" }} />
              </div>
              <div>
                <p style={{ fontSize: 12.5, fontWeight: 600, color: "rgba(255,255,255,0.72)", margin: "0 0 2px" }}>Booster mes annonces</p>
                <p style={{ fontSize: 11, color: "rgba(255,255,255,0.28)", margin: 0 }}>+300% de visibilité</p>
              </div>
            </div>
          </Link>
        </div>
