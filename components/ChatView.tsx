...
function OfferCard({
  ...
  const statusColors: Record<string, string> = {
    ...
    pending: "border-[#8B5CF6]/30 bg-[#8B5CF6]/8",
  ...
  return (
    <div className={cn("rounded-2xl border p-4 max-w-[280px] w-full", statusColors[offer.status] ?? statusColors.pending)}>
      ...
      <div className="flex items-center gap-2 mb-3">
        <div className="w-7 h-7 rounded-full bg-[#8B5CF6]/25 flex items-center justify-center text-[14px]">💰</div>
        <div>
          ...
          {showCounter && (
            <div className="flex gap-2 items-center mt-1">
              <input
                ...
                placeholder="Montant €"
                className="flex-1 px-3 py-2 rounded-xl bg-white/6 border border-white/10 text-[13px] text-white outline-none focus:border-[#8B5CF6]/50 placeholder-white/20"
              />
              <button
                onClick={() => {
                ...
                className="px-3 py-2 rounded-xl bg-[#8B5CF6] text-white text-[12px] font-bold active:scale-95 disabled:opacity-40"
              >
                Envoyer
              </button>
            </div>
        ...
        <div className="px-4 pb-2 flex gap-2 overflow-x-auto no-scrollbar animate-fadeIn">
          {QUICK_REPLIES.map((r) => (
            <button
              ...
              className="flex-shrink-0 px-3 py-1.5 rounded-full text-[12px] font-semibold text-[#A78BFA] border border-[#8B5CF6]/35 bg-[#8B5CF6]/8 active:scale-95 transition-all whitespace-nowrap"
            >
              {r}
            </button>
          ))}
      ...
      >
        <div className="flex items-center gap-2.5">
          <input
            ...
            className="flex-1 bg-white/6 border border-white/8 rounded-2xl px-4 py-3 text-[14px] text-white placeholder:text-white/25 outline-none focus:border-[#8B5CF6]/50 transition-all"
          />
          <button
            onClick={() => sendMessage()}
            ...
            className={cn(
              ...
              input.trim() ? "text-white shadow-lg shadow-[#8B5CF6]/25" : "bg-white/5 text-white/20"
            )}
            style={input.trim() ? { background: "linear-gradient(135deg, #8B5CF6, #7C3AED)" } : {}}
          >
