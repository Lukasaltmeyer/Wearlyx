  ...
  return (
    <div className="relative">
      ...
      <input value={value} onChange={e => onChange(e.target.value)} placeholder={placeholder ?? "Search…"}
        className="bg-[#0C0C0F] border border-[#1C1C20] rounded-lg pl-9 pr-4 py-2 text-[13px] text-zinc-300 placeholder-zinc-700 focus:outline-none focus:border-[#6C63FF]/40 transition-colors w-full" />
    </div>
  );
}
...
function Sparkline({ data, color = "#6C63FF", height = 32 }: { data: number[]; color?: string; height?: number }) {
...
function BarChart({ data, color = "#6C63FF", height = 80 }: { data: { label: string; value: number }[]; color?: string; height?: number }) {
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-3">
        <Card className="p-5">
          ...
          <BarChart data={revenueBar} color="#6C63FF" />
        </Card>
          ...
          {["all","free","starter","pro","premium"].map(p => (
            <button key={p} onClick={() => setPlan(p)}
              className={cn("px-2.5 py-1 rounded-md text-[11px] font-semibold capitalize transition-colors",
                plan === p ? "bg-[#6C63FF] text-white" : "text-zinc-600 hover:text-zinc-300")}>
              {p}
            </button>
          ))}
        </div>
                    ...
                    <Td>
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          ...
                          className="px-2.5 py-1.5 rounded-lg bg-[#6C63FF]/10 border border-[#6C63FF]/20 text-[11px] font-semibold text-[#9B93FF] hover:bg-[#6C63FF]/20 transition-colors"
                        >
                          Modérer
                        </button>
                      </div>
        ...
        <div className="flex bg-[#0C0C0F] border border-[#1C1C20] rounded-lg p-1 gap-0.5">
          {["all","active","sold","reserved","deleted"].map(s => (
            <button key={s} onClick={() => setStatusFilter(s)}
              className={cn("px-2.5 py-1 rounded-md text-[11px] font-semibold capitalize transition-colors",
                statusFilter === s ? "bg-[#6C63FF] text-white" : "text-zinc-600 hover:text-zinc-300")}>
              {s}
            </button>
          ))}
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70" onClick={() => setSelected(null)}>
          <div className="bg-[#0F0F13] border border-[#1C1C20] rounded-2xl p-6 w-full max-w-lg mx-4 space-y-4" onClick={e => e.stopPropagation()}>
            ...
            <textarea value={adminNote} onChange={e => setAdminNote(e.target.value)}
              ...
              className="w-full bg-[#0C0C0F] border border-[#1C1C20] rounded-lg p-3 text-[13px] text-zinc-300 placeholder-zinc-700 focus:outline-none focus:border-[#6C63FF]/40 resize-none" rows={2} />
              ...
              {filtered.map((d: any) => {
                ...
                return (
                  <tr key={d.id} className={cn("hover:bg-[#141417] transition-colors",
                    ...
                    d.status === "solution_proposed" && "bg-[#6C63FF]/5 border-l-2 border-[#6C63FF]/30")}>
                    <Td>
                      ...
                      <button onClick={() => { setSelected(d); setAdminNote(d.admin_note ?? ""); }}
                        className={cn("px-2.5 py-1.5 rounded-lg text-[11px] font-semibold transition-colors",
                          needsAction
                            ...
                            : "bg-[#6C63FF]/10 border border-[#6C63FF]/20 text-[#9B93FF] hover:bg-[#6C63FF]/20")}>
                        Gérer
                      </button>
                    </Td>
                  </tr>
        ...
        <div className="flex bg-[#0C0C0F] border border-[#1C1C20] rounded-lg p-1 gap-0.5">
          {["all","pending","paid","shipped","delivered","refunded","cancelled"].map(s => (
            <button key={s} onClick={() => setSf(s)}
              className={cn("px-2.5 py-1 rounded-md text-[11px] font-semibold capitalize transition-colors",
                sf === s ? "bg-[#6C63FF] text-white" : "text-zinc-600 hover:text-zinc-300")}>
              {s === "all" ? "Tous" : STATUS_LABEL[s] ?? s}
            </button>
          ))}
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70" onClick={() => setSelected(null)}>
          <div className="bg-[#0F0F13] border border-[#1C1C20] rounded-2xl p-6 w-full max-w-md mx-4" onClick={e => e.stopPropagation()}>
            ...
            <textarea value={note} onChange={e => setNote(e.target.value)} placeholder="Note admin (optionnel)…"
              className="w-full bg-[#0C0C0F] border border-[#1C1C20] rounded-lg p-3 text-[13px] text-zinc-300 placeholder-zinc-700 focus:outline-none focus:border-[#6C63FF]/40 resize-none mb-4" rows={3} />
    ...
    return (
      <button key={id} onClick={() => { setView(id as View); setSidebarOpen(false); }}
        className={cn(
          ...
          active ? "bg-[#6C63FF]/12 text-[#9B93FF]" : "text-zinc-600 hover:text-zinc-300 hover:bg-[#0F0F13]"
        )}>
        <Icon className={cn("w-4 h-4 flex-shrink-0", active && "text-[#6C63FF]")} />
          ...
          {MOBILE_NAV.map(id => {
            ...
            return (
              <button key={id} onClick={() => setView(id as View)}
                ...
                {active && <span className="absolute top-0 left-1/2 -translate-x-1/2 w-8 h-0.5 rounded-full bg-[#6C63FF]" />}
              </button>
            );
          })}
        </nav>
