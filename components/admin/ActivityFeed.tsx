...
export function ActivityFeed({ recentUsers, recentProducts, recentOrders, recentOffers }: Props) {
  const events = [
    ...
    ...(recentProducts ?? []).map((p: any) => ({
      icon: "👕", label: `Annonce : ${p.title} — ${p.price}€`, date: p.created_at, color: "text-violet-400",
    })),
    ...(recentOrders ?? []).map((o: any) => ({
      icon: "🛒", label: `Vente : ${(o.product as any)?.title} (${o.total}€) par ${(o.buyer as any)?.username}`, date: o.created_at, color: "text-violet-400",
    })),
