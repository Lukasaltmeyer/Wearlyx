...
export function PromotionToolsClient({ products, isDesktop }: Props) {
  ...
  const productList = (
    <div className="flex flex-col gap-1.5">
      {products.length === 0 ? (
        <div className="py-10 text-center border border-white/6 rounded-[14px] bg-white/[0.02]">
        ...
        products.map((p) => {
          ...
          return (
            <div key={p.id} className="flex items-center gap-3 px-3 py-3 rounded-[12px] border border-white/6 bg-white/[0.02]">
              <div className="w-11 h-11 rounded-[10px] overflow-hidden flex-shrink-0 bg-white/5">
                {p.images?.[0] ? (
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[13px] font-semibold text-white/85 line-clamp-1">{p.title}</p>
                <p className="text-[12px] text-white/35">{formatPrice(p.price)}</p>
              </div>
              <button onClick={() => !isBoosted && setBoosted((prev) => [...prev, p.id])}
                className={`flex items-center gap-1 px-3 py-1.5 rounded-[8px] text-[12px] font-bold transition-all ${
                  isBoosted ? "bg-[#3B82F6]/20 text-[#3B82F6]" : "bg-[#3B82F6] text-white hover:bg-[#3B82F6]/90"
                }`}>
  ...
  if (isDesktop) {
    return (
      <div className="flex flex-col gap-6">
        <div>
          <p className="text-[11px] font-semibold text-white/25 uppercase tracking-widest mb-3">Booster une annonce</p>
          {productList}
        </div>
        <div>
          <p className="text-[11px] font-semibold text-white/25 uppercase tracking-widest mb-3">Offres</p>
          <div className="flex items-center gap-3 px-4 py-4 rounded-[14px] border border-white/6 bg-white/[0.02]">
            <div className="w-10 h-10 rounded-[10px] bg-[#4CAF50]/12 flex items-center justify-center flex-shrink-0">
              <Percent className="w-5 h-5 text-[#4CAF50]" />
            </div>
            <div className="flex-1">
              <p className="text-[14px] font-semibold text-white/80">Réduction sur les lots</p>
