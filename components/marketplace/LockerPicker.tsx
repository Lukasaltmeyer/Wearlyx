          ...
          return (
            <button
              ...
              className={cn(
                ...
                mode === m ? "border-[#6C63FF] bg-[#6C63FF]/10" : "border-white/8 bg-white/3"
              )}
            >
              <Icon className={cn("w-5 h-5", mode === m ? "text-[#6C63FF]" : "text-white/40")} />
              <span className={cn("text-[12px] font-bold", mode === m ? "text-white" : "text-white/40")}>{labels[m]}</span>
              <span className={cn("text-[10px] font-semibold", mode === m ? "text-[#6C63FF]" : "text-white/25")}>
        ...
        <div className="flex flex-col gap-3">
          ...
          {[
            ...
            <input
              ...
              className="w-full px-4 py-3 rounded-2xl bg-white/6 border border-white/10 text-[14px] text-white placeholder-white/25 outline-none focus:border-[#6C63FF]/50 transition-colors"
            />
          ))}
        </div>
      )}
      ...
      {(mode === "locker" || mode === "relay") && (
        <>
          ...
          <div className="flex gap-2">
            <div className="flex-1 flex items-center gap-2 px-3 py-2.5 rounded-2xl bg-white/6 border border-white/10 focus-within:border-[#6C63FF]/50 transition-colors">
            ...
            <button
              ...
              disabled={loading}
              className="px-3 py-2.5 rounded-2xl bg-[#6C63FF]/20 border border-[#6C63FF]/30 text-[#a78bfa] text-[12px] font-bold flex-shrink-0"
            >
              Chercher
            </button>
            <button
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      filter === f ? "bg-[#6C63FF] text-white" : "bg-white/6 text-white/40 border border-white/10"
                    )}
                  >
                    {labels[f]} ({count})
                  </button>
          ...
          {!loading && !geoError && points.length === 0 && (
            <div className="flex flex-col items-center py-8 gap-3 text-center">
              ...
              <button
                onClick={geolocate}
                className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-[#6C63FF]/15 border border-[#6C63FF]/30 text-[#a78bfa] text-[13px] font-semibold"
              >
              ...
              {filteredPoints.map((point) => (
                ...
                >
                  <div className={cn(
                    ...
                    point.type === "locker" ? "bg-purple-500/20" : "bg-blue-500/20"
                  )}>
                    {point.type === "locker"
                  ...
                  <div className="flex flex-col items-end gap-1 flex-shrink-0">
                    ...
                    {selected?.id === point.id && (
                      <span className="text-[10px] font-black text-[#6C63FF]">✓</span>
                    )}
                  </div>
                </button>
              ))}
